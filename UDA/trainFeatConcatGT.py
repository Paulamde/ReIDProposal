        ...
        for epoch in range(32):
            ...
            if epoch % 32 == 0:  # and epoch < 9) or (epoch % 6 == 0):
                target_features, target_labels, target_camids, target_trkids = extract_features(model, val_loader,
                                                                                                print_freq=100)
                target_features_train, target_labels_train, target_camids_train, target_trkids_train = extract_features(model, train_loader_train,
                                                                                                print_freq=100)
                ...
                P_train, neg_vec_train = compute_P2(target_features_train, target_features_train, target_camids_train, la= 0.0005)
                ...
                for i, trkid in enumerate(unique_trkids_train):
                     track_feature_train = torch.mean(target_features_train[target_trkids_train == trkid], dim=0, keepdim=True)
                     tmp_indices = (target_trkids_train == trkid)
                     target_features_train[tmp_indices] = target_features_train[tmp_indices] * 0.3 + track_feature_train * 0.7
                ...
                write_line = 'number of IDs: '+str(labelset.__len__())+' Number of images: '+str(idxs[0].size) + '\n'
