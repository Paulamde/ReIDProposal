...
tree = ET.parse('D:/2019-CVPR-AIC-Track-2-UWIPL-master/AICity_SyNew'
                '/train_label.xml', parser=xmlp)
root = tree.getroot()
...
aic_track2_dir = 'D:/2019-CVPR-AIC-Track-2-UWIPL-master/AICity_SyNew//'
...
if not os.path.isdir(path_train):
    os.mkdir(path_train)
if not os.path.isdir(path_query):
    os.mkdir(path_query)
if not os.path.isdir(path_query_single):
    os.mkdir(path_query_single)
if not os.path.isdir(path_gallery):
    os.mkdir(path_gallery)
file = open(aic_track2_dir+"train_track_id.txt","r")
for line in file:
    print(line)
    s1 = line.replace(" \n","").split(" ")
    if s1[0]=='\n':
