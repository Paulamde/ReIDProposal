...
def dump_matches_imgids(output_dir, matches_imgids):
    if not osp.isdir(output_dir):
        mkdir(output_dir)
    for q_imgid, g_imgids in matches_imgids.items():
...
def dump_query_result(output_dir, matches_imgids, top_N=100):
    ...
    with open(osp.join(output_dir, 'track2.txt'), 'w') as f:
        for q_imgid, g_imgids in sorted(matches_imgids.items()):
...
def evaluate_imgids(distmat, feat, q_pids, g_pids, q_camids, g_camids, q_imgids, g_imgids, max_rank=50, top_N=0):
    '''
   ...
   #Paula
    #aux = np.zeros((11579, 2048))
    aux = np.zeros((18290, 2048))

    cont = 0
    for i in g_counts:
        for j in range(i):
            indx = int(g_imgids[cont][j])
            aux[indx - 1] = feat[cont]
        cont = cont + 1
    np.save('gf_feature4_SynewtestAiCity.npy', aux)
    ...
    # count gt and prediction (first imgid only) VAMOS QUE SOLO LA PRIMEAA IMAGEN DEL TRACK DE TEST
    matches_gt_pred = {}
    ...
    for q_idx in range(num_q):
        ...
        for gi_idx in range(num_gi):
            ...
            # if (g_pids[gi_idx] == q_pids[q_idx]) & (g_camids[gi_idx] == q_camids[q_idx]):
            #     aux = 1
            # else:
    ...
    for q_idx in range(num_q):
        ...
        #PAULA
        auxiliar1=order[keep]
        index = g_pids[auxiliar1]
        good_index = np.where(index==q_pid)
        ap_tmp = compute_mAP(index, good_index)
        ap_Paula += ap_tmp
    ...
    mAP = np.mean(all_AP)
    mAP_Paula = ap_Paula / num_q
    return all_cmc, mAP, matches_imgids, matches_imgids_FP, matches_gt_pred,mAP_Paula,feat_result

def compute_mAP(index, good_index):
    index = np.asarray(index)
    good_index = np.asarray(good_index)
    ap = 0
    #cmc = torch.IntTensor(11579).zero_()
    cmc = torch.IntTensor(18290).zero_()
