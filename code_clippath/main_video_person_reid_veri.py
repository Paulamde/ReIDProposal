...
parser.add_argument('-d', '--dataset', type=str, default='mars',
                    choices=data_manager_sy.get_names())
...
parser.add_argument('--max-epoch', default=305, type=int,
...
def main():
    ...
    dataset = data_manager_sy.init_dataset(name=args.dataset)
...
def evaluate_feature(qf, gf, q_metadatas, g_metadatas, q_pids, g_pids, q_camids, g_camids, q_imgids, g_imgids, q_imgpaths, g_imgpaths, aggregate_writer, epoch, ranks):
    ...
    if args.metadata_model:
        ...
        confusion_file = 'D:/2019-CVPR-AIC-Track-2-UWIPL-master/Video-Person-ReID/metadata/cm_%s_normalized.npy'%args.metadata_model
        ...
        if args.metadata_model[:2] == 'v1': 
            ...
            for i, prob_range in enumerate(default_metadata_prob_ranges):
                confusion_mats[prob_range] = confusion_mats_obj[i]
        elif args.metadata_model[:2] == 'v3':
            default_metadata_prob_ranges = [(0,8), (8,37), (37,46)]
            confusion_mats = dict()
            for i, prob_range in enumerate(default_metadata_prob_ranges):
                confusion_mats[prob_range] = confusion_mats_obj[i]
        else:
    ...
    if MemorySave:
        ...
        gf_feat = np.zeros((18290, 2048))
        #gf_feat = np.zeros((11579, 2048))
        for g_idx in range(gallery_num):
            g_im_index = g_imgids[g_idx]
            g_im_index = np.asarray(g_im_index)
            sizeRange = np.size(g_im_index)
            for g_index in range(sizeRange):
                aux = int(g_im_index[g_index])
                gf_feat[aux-1] = gf[g_idx]
    ...
    metadata_dist = conf_pred * confusion_dist #solo hace algo si activamos KL
    ...
    #PAULA
    np.save('qf_feature4_AicitytrainVeRiSy.npy', qf)
    np.save('gf_feature4_AicitytrainVeRiSy.npy', gf)
    cmc, mAP, matches_imgids, matches_imgids_FP, matches_gt_pred,mAP_Paula ,feat_result= evaluate_imgids(distmat, gf, q_pids, g_pids, q_camids, g_camids, q_imgids, g_imgids, 50, 100)
    print("Results ----------")
    print("mAP: {:.1%}".format(mAP))
    print("mAP paula: {:.1%}".format(mAP_Paula))
    print("CMC curve")
    for r in ranks:
        ...
        for k1 in range(4,5,1):
            for k2 in range(4,k1+1,1):
                ...
                for lambda_value in np.arange(0.5,0.6,0.1):
                    ...
                    for r_metadata in np.arange(0.6, 0.61, 0.01):
                    ...
                    # PaulaCOment
                    #cmc, mAP, matches_imgids, matches_imgids_FP, matches_gt_pred, mAP_Paula,feat_result = evaluate_imgids(final_dist,gf,  q_pids, g_pids, q_camids, g_camids, q_imgids, g_imgids, 50, 100)
                        print("after re-ranking Results ----------")
                        print("after re-ranking: mAP: {:.1%}".format(mAP))
                        print("after re-ranking: mAP PAULA: {:.1%}".format(mAP_Paula))
